...
const AUTH_ERROR_MESSAGES: Record<string, string> = {
  ...
  AccountLocked: "Your account has been temporarily locked due to too many failed attempts. Please try again in 15 minutes or reset your password.",
...
export default function LoginPage() {
  ...
  const error = formError ?? (authError ? AUTH_ERROR_MESSAGES[authError] || "An authentication error occurred." : null)
  ...
  const onOAuthSignIn = async (provider: "google" | "github") => {
    ...
    try {
      ...
      setFormError("Unable to continue with OAuth provider. Please try again.")
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <Link href="/forgot-password" className="text-sm font-medium text-brand-default hover:text-brand-hover">
              Forgot password?
            </Link>
          </div>
      ...
      <div className="mt-6">
        <div className="relative">
          <div className="absolute inset-0 flex items-center">
            <div className="w-full border-t border-line-default"></div>
          </div>
          <div className="relative flex justify-center text-sm">
            <span className="bg-bg-base px-2 text-text-tertiary">or continue with</span>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4">
          <Button
            type="button"
            variant="ghost"
            className="border border-line-default hover:bg-bg-surface"
            disabled={isLoading}
      ...
      <p className="mt-10 text-center text-sm text-text-secondary">
        ...
        <Link href="/register" className="font-semibold leading-6 text-brand-default hover:text-brand-hover">
          Sign up
        </Link>
      </p>
    </div>
